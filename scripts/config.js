...
const mandatoryVariables = settings => {
  const clientIdDefaultMaybe =
    settings &&
    settings.VITE_SHARETRIBE_SDK_CLIENT_ID !== '' &&
    settings.VITE_SHARETRIBE_SDK_CLIENT_ID !== 'change-me'
      ? { default: settings.VITE_SHARETRIBE_SDK_CLIENT_ID }
      : {};
  ...
  const stripeDefaultMaybe =
    settings && settings.VITE_STRIPE_PUBLISHABLE_KEY !== ''
      ? { default: settings.VITE_STRIPE_PUBLISHABLE_KEY }
      : {};
  const mapBoxDefaultMaybe =
    settings && settings.VITE_MAPBOX_ACCESS_TOKEN !== ''
      ? { default: settings.VITE_MAPBOX_ACCESS_TOKEN }
      : {};
  ...
  return [
    {
      type: 'input',
      name: 'VITE_SHARETRIBE_SDK_CLIENT_ID',
...
const advancedSettings = settings => {
  const rootUrlDefault = settings ? settings.VITE_MARKETPLACE_ROOT_URL : null;
  const cspDefault = settings ? settings.VITE_CSP : null;
  ...
  return [
    ...
    {
      type: 'input',
      name: 'VITE_MARKETPLACE_ROOT_URL',
...
`,
    ...
    {
      type: 'input',
      name: 'VITE_APP_MARKETPLACE_NAME',
      message: `What is the name of your marketplace?
${chalk.dim(
  'The marketplace name is needed for the marketplace texts. If not set through environment variables, the name defaults to "Biketribe" (set in src/config/configDefault.js)'
)}
