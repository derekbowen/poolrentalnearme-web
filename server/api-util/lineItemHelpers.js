...
const LINE_ITEM_AMENITY = 'line-item/amenity-';
const LINE_ITEM_REFUNDABLE_DEPOSIT = 'line-item/refundable-deposit';

exports.LINE_ITEM_AMENITY = LINE_ITEM_AMENITY;
exports.LINE_ITEM_REFUNDABLE_DEPOSIT = LINE_ITEM_REFUNDABLE_DEPOSIT;
...
exports.getAmenityLineItemsWithName = (lineItems, listing) => {
  const { amenities } = listing.attributes.publicData;
  const amenityLineItems = lineItems.filter((item) => item.code.includes(LINE_ITEM_AMENITY));
  return amenityLineItems.map((item) => {
    const amenityId = item.code.replace(LINE_ITEM_AMENITY, '');
...
exports.getAmenityLineItemsWithNameInUnit = (lineItems, listing) => {
  const { amenities } = listing.attributes.publicData;
  const amenityLineItems = lineItems.filter((item) => item.code.includes(LINE_ITEM_AMENITY));
  ...
  return amenityLineItems.map((item) => {
    ...
    const amenity = amenities.find((a) => a.id === amenityId);
    const { unitPrice } = item;
    const subunitDivisor = unitDivisor(unitPrice.currency);
    const amount = subunitDivisor ? unitPrice.amount / subunitDivisor : unitPrice.amount;
    return {
      ...item,
      unitPrice: {
        amount,
