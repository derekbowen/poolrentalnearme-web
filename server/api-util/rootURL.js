...
/**
 * This function uses VITE_MARKETPLACE_ROOT_URL by default,
 * Note: on dev/localhost setup this returns hostname to apiServer
 * I.e. if VITE_DEV_API_SERVER_PORT is used, the port is changed to it: e.g. http://localhost:3500
 *
 * @param {Object} options can contain options like rootURL and useDevApiServerPort. If rootURL is undefined, function uses VITE_MARKETPLACE_ROOT_URL
 * @returns hostname
 */
exports.getRootURL = (options) => {
  const { rootURL, useDevApiServerPort = false } = options || {};
  const protocol = !useDevApiServerPort ? 'https' : 'http';
