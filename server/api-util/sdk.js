...
const ASSET_CDN_BASE_URL = process.env.VITE_SHARETRIBE_SDK_ASSET_CDN_BASE_URL;
...
exports.getSdk = (req, res, opts = {}) => {
  const { alwaysDenormalised = false, ...restOpts } = opts;
  const instance = sharetribeSdk.createInstance({
  ...
  return alwaysDenormalised ? wrapInstanceWithResponseTransformer(instance) : instance;
};
...
exports.fetchMarketplaceContent = (sdk) => {
  return sdk
    .assetsByAlias({ paths: ['/content/translations.json'], alias: 'latest' })
    .then((response) => {
      // Let's throw an error if we can't fetch branding for some reason
      const marketplaceContent = response?.data?.data?.[0];
      if (!marketplaceContent) {
        const message = 'marketplace-content configuration was not available.';
        const error = new Error(message);
        error.status = 404;
        error.statusText = message;
        error.data = {};
