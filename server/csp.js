...
const baseUrl = process.env.VITE_SHARETRIBE_SDK_BASE_URL || 'https://flex-api.sharetribe.com';
...
const assetCdnBaseUrl = process.env.VITE_SHARETRIBE_SDK_ASSET_CDN_BASE_URL;
...
const defaultDirectives = {
  baseUri: [self],
  defaultSrc: [self, 'hero-banner.s3.us-west-1.amazonaws.com'],
  childSrc: [blob],
  connectSrc: [
    self,
    baseUrl,
...
exports.csp = (reportUri, reportOnly) => {
  ...
  const directives = merge({ reportUri: [reportUri], ...defaultDirectives }, customDirectives);
  if (!reportOnly && !dev) {
