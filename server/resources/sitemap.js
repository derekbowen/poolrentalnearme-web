...
const { default: queryAllPage } = require('extensions/common/utils/queryAllPage.js');
...
const defaultPublicPaths = {
  ...
  hostPreparednessPolicy: { url: '/host-standards' },
...
const sitemapListings = (req, res, rootUrl, sdk) => {
  ...
  queryAllPage({ sdkModel: sdk.listings })
    .then((listings) => {
...
const handleSitemaps = (req, res, next, sdk, isPrivateMarketplace) => {
  ...
  if (sitemapResource === 'index') {
    sitemapIndex(req, res, rootUrl, isPrivateMarketplace);
