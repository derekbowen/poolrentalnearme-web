...
const estimatedCustomerTransaction = (
  ...
  const amenityLineItemsWithName = amenityLineItems.map((item) => {
    const amenity = amenities.find((a) => a.id === item.code.split(LINE_ITEM_AMENITY)[1]);
    return {
      ...item,
      name: amenity?.name,
    };
  ...
  return {
    ...
    attributes: {
      ...
      protectedData: {
        amenityLineItems: amenityLineItemsWithName,
      },
    },
    ...bookingMaybe,
  };
