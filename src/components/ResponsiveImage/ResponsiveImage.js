...
import heroVideoUrl from '../../extensions/hero/config';
...
const ResponsiveImage = props => {
  ...
  const enableVideoBackground =
    intl.formatMessage({ id: 'LandingPage.enableVideoBackground' }) === 'true';
  ...
  if (isLandingPage && isSectionBackground && enableVideoBackground) {
    return (
      <video autoPlay loop muted playsInline className={classes}>
        <source src={heroVideoUrl} type="video/mp4" />
        {intl.formatMessage({ id: 'ResponsiveImage.videoNotSupported' })}
      </video>
    );
  }
