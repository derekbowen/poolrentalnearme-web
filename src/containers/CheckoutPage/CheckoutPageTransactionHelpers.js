...
import { createPaymentMethodAndSave } from '../../extensions/off-session-payment/ducks/offSessionPayment.duck';
...
export const processCheckoutWithPayment = (orderParams, extraPaymentParams) => {
  ...
  const fnRequestPayment = (fnParams) => {
    ...
    const requestTransition = process.getRequestTransition(storedTx);
 ...
 * @returns Promise that goes through each step in the checkout sequence.
 */
export const processCheckoutOffSessionPayment = ({ orderParams, extraPaymentParams, dispatch }) => {
  const {
    message,
    onInitiateOrder,
    onSendMessage,
  ...
  //////////////////////////////////////////////////////////
  // Step 3: save card as defaultPaymentMethod //
  //////////////////////////////////////////////////////////
  const fnSavePaymentMethod = async (fnParams) => {
    if (isPaymentFlowPayAndSaveCard && !isPaymentFlowUseSavedCard) {
      const { stripe, card, billingDetails } = extraPaymentParams;
      const savedPaymentMethod = await dispatch(
        createPaymentMethodAndSave({
          stripe,
          card,
          billingDetails,
