...
import { LINE_ITEM_HOUR, DATE_TYPE_DATE, DATE_TYPE_DATETIME } from '../../util/types';
...
import {
  ...
  processCheckoutOffSessionPayment,
...
const fetchSpeculatedTransactionIfNeeded = (orderParams, pageData, fetchSpeculatedTransaction) => {
  ...
  if (shouldFetchSpeculatedTransaction) {
    ...
    const requestTransition = process.getRequestTransition(tx);
...
export const loadInitialDataForStripePayments = ({
  ...
  const currentListing = pageData.listing;
  const authorId = currentListing?.author.id.uuid;
  dispatch(fetchStripeConnected(authorId));
...
const handleSubmit = async (values, process, props, stripe, submitting, setSubmitting) => {
  ...
  const forcedToSavedCard = !stripeConnected;

  const isPaymentFlowPayAndSaveCard =
    selectedPaymentFlow === PAY_AND_SAVE_FOR_LATER_USE || forcedToSavedCard;
  ...
  // which is either initiate-transition or initiate-transition-after-enquiry
  const orderParams = getOrderParams(pageData, shippingDetails, optionalPaymentParams, config);
  const { isInstantBooking, transactionProcessAlias } =
    pageData.listing.attributes.publicData || {};
  const isNotBooking = !isBookingProcessAlias(transactionProcessAlias);
  if (stripeConnected || isInstantBooking || isNotBooking) {
    ...
    const response = await processCheckoutOffSessionPayment({
      orderParams,
      extraPaymentParams: requestPaymentParams,
      dispatch,
    });
    const { orderId, messageSuccess, paymentMethodSaved } = response;
    setSubmitting(false);
