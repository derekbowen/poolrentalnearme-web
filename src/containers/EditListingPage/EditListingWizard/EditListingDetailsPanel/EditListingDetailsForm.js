  ...
  <FinalForm
    ...
    render={formRenderProps => {
      ...
      const renderLabel = ({ value }) => {
        const advantage = advantages.find((ad) => ad.id === value);
        if (!advantage) {
          return null;
        }
        const { title, url } = advantage;
        return (
          <div className={css.checkboxLabel}>
            <img src={url} alt={title} className={css.checkboxImage} />
            <span>{title}</span>
          </div>
        );
      };

      const advantagesOptions = advantages.map((advantage) => ({
        key: advantage.id,
        ...
        <Form className={classes} onSubmit={handleSubmit}>
          ...
          {listingType && (
            <FieldCheckboxGroup
              ...
              id="advantages"
              name="advantagesSelection"
              label={intl.formatMessage({ id: 'EditListingDetailsForm.advantagesLabel' })}
