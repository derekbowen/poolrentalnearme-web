...
const getFieldConfig = (data, defaultComponents, options) => {
  const customFieldComponents = options?.fieldComponents || {};
  const fieldMapping = { ...defaultComponents, ...customFieldComponents };
...
export const validProps = (data, options) => {
  ...
  if (data && pickValidProps) {
    const validatedProps = pickValidProps(data);
    ...
    if (Object.keys(validatedProps).length === 0 && !omitWarning) {
