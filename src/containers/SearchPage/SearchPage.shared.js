...
export const cleanSearchFromConflictingParams = (searchParams, filterConfigs, sortConfig) => {
  ...
  const isKeywordSearchActive = searchParams.keywords && searchParams.keywords.length > 0;
  const { sort } = searchParams;
  const isDistanceSortActive = sort === SORT_BY_DISTANCE;
  ...
  if (isKeywordSearchActive && isDistanceSortActive) {
    return { ...unlimitedSearchParams, sort: null, origin: null };
  }
...
export const searchParamsPicker = (
  ...
  const searchParamsAreInSync =
    stringify(omit(queryParamsFromURL, 'origin')) ===
    stringify(omit(queryParamsFromSearchParams, 'origin'));
