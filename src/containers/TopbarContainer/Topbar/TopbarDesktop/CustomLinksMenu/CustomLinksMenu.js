...
const CustomLinksMenu = ({
  ...
  useEffect(() => {
    ...
    const { body } = document;
    if (container && hasClientSideContentReady && moreLabelWidth > 0) {
      ...
      observer.current = new ResizeObserver(entries => {
        ...
        for (const entry of entries) {
          ...
          if (isBodyTheTarget || hasWidthOfTopbarDesktopChanged) {
            const availableContainerWidth = calculateContainerWidth(container, bodyOffsetWidth);
  ...
  }, [hasClientSideContentReady, moreLabelWidth, links]);
