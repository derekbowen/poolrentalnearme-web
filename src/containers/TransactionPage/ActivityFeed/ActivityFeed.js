...
export const ActivityFeed = (props) => {
  ...
  const { paymentFailedAt } = transaction?.attributes?.metadata || {};
  ...
  return (
    <ul className={classes}>
      ...
      {paymentFailedAt && isWaitingForPayment ? (
        <Transition
          formattedDate={formatDateWithProximity(new Date(paymentFailedAt), intl, todayString)}
          transitionMessageComponent={intl.formatMessage({
            id: `ActivityFeed.paymentFailed.${getUserTxRole(currentUser.id, transaction)}`,
          })}
        />
      ) : null}
    </ul>
