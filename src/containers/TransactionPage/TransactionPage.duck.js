...
import { fetchStripeConnected } from '../../extensions/off-session-payment/ducks/offSessionPayment.duck';
...
import { acceptWithPayment } from '../../extensions/off-session-payment/api';
...
export const makeAcceptWithPaymentTransition =
  ({ txId, params }) =>
  async (dispatch, getState, sdk) => {
    try {
      const response = await acceptWithPayment({ params, txId });
      dispatch(addMarketplaceEntities(response));
      ...
      return dispatch(
        transitionError(storableError({ ...e, lastTransition: transitions.ACCEPT_WITH_PAYMENT }))
      );
    }
  };
...
export const makeTransition = (txId, transitionName, params) => (dispatch, getState, sdk) => {
  ...
  if (transitionName === transitions.ACCEPT_WITH_PAYMENT) {
    return dispatch(makeAcceptWithPaymentTransition({ txId, params }));
  }
  ...
  return sdk.transactions
    ...
    .catch((e) => {
      dispatch(transitionError(storableError({ ...e, lastTransition: transitionName })));
  ...
  return sdk.transactions
    ...
    .catch((e) => {
      ...
      if (isTransactionsTransitionInvalidTransition(e)) {
        return sendReviewAsSecond(txId, secondReviewTransition, params, dispatch, sdk, config);
      }
      dispatch(sendReviewError(storableError(e)));
...
export const loadData = (params, search, config) => (dispatch, getState) => {
  ...
  return Promise.all([
    ...
    dispatch(fetchStripeConnected(currentUser?.id.uuid)),
  ]);
};

export const havePaymentErrors = (state) => {
  const { transitionError: error } = state.TransactionPage;
  return error;
};
