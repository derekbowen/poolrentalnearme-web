...
import { selectStripeConnected } from '../../extensions/off-session-payment/ducks/offSessionPayment.duck';
...
export const TransactionPageComponent = (props) => {
  ...
  const stripeConnected = useSelector(selectStripeConnected);
  ...
  const isTxOnPaymentPending = useCallback(
    (tx) => {
  ...
  const redirectToCheckoutPageWithInitialValues = useCallback((initialValues, currentListing) => {
...
const TransactionPage = connect(mapStateToProps, mapDispatchToProps)(TransactionPageComponent);
