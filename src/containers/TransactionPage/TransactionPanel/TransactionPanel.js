...
import { LISTING_TYPE_HOURLY } from 'config/configListing';
  ...
  render() {
    ...
    const actionButtons = (
      <ActionButtonsMaybe
        ...
        disabledActionButtons={!stripeConnected}
      />
    );
    ...
    const timezone = get(listing, 'attributes.availabilityPlan.timezone') || 'Etc/UTC';

    const { start, end } = get(transaction, 'booking.attributes', {});

    const startTime = formatTime(start, timezone, 'HH:mm');
    const endTime = formatTime(end, timezone, 'HH:mm');

    const startDate = formatTime(start, timezone, 'YYYY-MM-DD');
    const endDate = formatTime(end, timezone, 'YYYY-MM-DD');

    const { lastTransition } = transaction?.attributes || {};

    const lastTransitionsToShowCalendar = [
      transitions.CONFIRM_PAYMENT,
      transitions.ACCEPT,
      transitions.OPERATOR_ACCEPT,
    ];

    const isShowCalendarBtn =
      listingType === LISTING_TYPE_HOURLY && lastTransitionsToShowCalendar.includes(lastTransition);
    const anchor = (chunks) => (
      <NamedLink name="StripePayoutPage" className={css.payoutSetting}>
        {chunks}
      </NamedLink>
    );
      ...
      <div className={classes}>
        <div className={css.container}>
          <div className={css.txInfo}>
            ...
            />
            {!stripeConnected && isProvider ? (
              <p className={css.stripeNotConnectedInfoText}>
                {intl.formatMessage(
                  { id: 'TransactionPanel.stripeNotConnected' },
                  {
                    a: anchor,
                  }
                )}
            ...
            {isProvider ? (
              <div
                className={classNames({
                  [css.avatarWrapperProviderDesktop]: true,
                  [css.stripeNotConnected]: !stripeConnected,
                })}
              >
