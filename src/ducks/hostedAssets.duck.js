import { LISTING_TYPE_CONFIG } from 'config/configListing';
import { denormalisedResponseEntities, denormalizeAssetData } from '../util/data';
...
export const appAssetsSuccess = (assets, version, googleAnalyticsId, advantages) => ({
  type: ASSETS_SUCCESS,
  payload: { assets, version, googleAnalyticsId, advantages },
});
...
export const advantagesSelector = (state) => state.hostedAssets.advantages;
...
export const fetchAppAssets = (assets, version) => (dispatch, getState, sdk) => {
  ...
  const getAdminListing = () =>
    sdk.listings.query({
      pub_listingType: LISTING_TYPE_CONFIG,
    });
  ...
  return Promise.all(separateAssetFetches)
    .then(([translationAsset, footerAsset, configAssets, adminAssets]) => {
      ...
      const [adminListing] = denormalisedResponseEntities(adminAssets);
      const { advantages } = adminListing?.attributes?.publicData || {};
      ...
      dispatch(appAssetsSuccess(assets, versionInTranslationsCall, googleAnalyticsId, advantages));
