...
import { getAllExtensionTransactionFile } from '../extension';

const extensionProcess = await getAllExtensionTransactionFile();
...
export const getAllTransitionsForEveryProcess = () => {
  return PROCESSES.reduce((accTransitions, processInfo) => {
    const { transitions, excludeTransitions = [] } = processInfo.process;
    return [
      ...accTransitions,
      ...Object.values(transitions).filter((t) => !excludeTransitions.includes(t)),
    ];
  }, []);
};
