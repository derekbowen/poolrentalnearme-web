  ...
  return listingFields.reduce((acc, data) => {
    ...
    const validationData = Object.entries(data).reduce(
      (acc, entry) => {
        ...
        if (isValid === false && typeof window !== 'undefined') {
          console.warn(
...
const createExclusionFieldFilter = (excludedListingFields) => (field) =>
  excludedListingFields.some((excludedField) =>
    typeof excludedField === 'function' ? excludedField(field.key) : field.key === excludedField
  );

/**
 * Retrieves the available listing fields from the given configuration.
 *
 * @param {Object} config - The configuration object.
 * @param {Object} config.listing - The listing configuration object.
 * @param {Array} [config.listing.listingFields=[]] - The array of listing fields.
 * @param {Array} [config.listing.excludedListingFields=[]] - The array of excluded listing fields.
 * @returns {Array} - An array of available listing fields.
 */
export const getAvailableListingFields = (config) => {
  if (!config?.listing) return [];

  const { listingFields = [], excludedListingFields = [] } = config.listing;

  const isFieldExcluded = createExclusionFieldFilter(excludedListingFields);

  return listingFields.filter((field) => !isFieldExcluded(field));
};
