...
export const transactionInitiateOrderStripeErrors = error => {
  if (error) {
    return errorAPIErrors(error).reduce((messages, apiError) => {
      const isMissingStripeAccountError =
        apiError.code === ERROR_CODE_MISSING_STRIPE_ACCOUNT && apiError.status === 409;
      if (isMissingStripeAccountError && apiError.title) {
        return [...messages, apiError.title];
      }
      const isPaymentFailedError =
        apiError.status === 402 && apiError.code === ERROR_CODE_PAYMENT_FAILED;
...
export const storableError = err => {
  const error = err || {};
  const { name, message, status, statusText, lastTransition } = error;
  ...
  if (lastTransition) {
    reconstructedError.lastTransition = lastTransition;
  }
...
export const isExceedRateLimitError = (error) => {
  return hasErrorWithCode(error, EXCEED_RATE_LIMIT_CODE);
};

export const isInvalidOTPError = (error) => {
  return hasErrorWithCode(error, INVALID_OTP_CODE);
};
