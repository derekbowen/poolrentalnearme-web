  ...
  new Promise((resolve, reject) => {
    ...
    service.getDetails({ placeId, fields, ...sessionTokenMaybe }, (place, status) => {
      if (status !== serviceStatus.OK) {
        ...
        const administrativeAreaLv1 = addresses.find((ad) =>
          ad.types.includes('administrative_area_level_3')
        );

        const administrativeAreaLv1Name = administrativeAreaLv1
          ? administrativeAreaLv1.long_name
          : '';
        resolve({
          address: place.formatted_address,
          origin: placeOrigin(place),
          bounds: placeBounds(place),
          city: administrativeAreaLv1Name,
        });
      }
    });
  });
...
export const getPlacePredictions = (search, sessionToken, searchConfigurations) =>
  new Promise((resolve, reject) => {
    ...
    console.info('#trang sercive', 'getPlacePrefic Google', searchConfigurations);
    ...
    service.getPlacePredictions(
      ...
      (predictions, status) => {
        if (!predictionSuccessful(status)) {
          ...
          console.info('#trang predictions', predictions);
