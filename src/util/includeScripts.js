...
const { VITE_FACEBOOK_PIXEL_ID } = import.meta.env;
  ...
  fbPixelScript.push(
    <script key="function" id="facebook-pixel-script">
      {`
        ...
        const isFacebookAd = urlParams.get('utm_source') === 'facebook';
        if (isFacebookAd || document.referrer.includes("facebook.com")) {
           fbq('trackCustom', 'FacebookAdPageView', 
            { date: new Date().toISOString(),
              userAgent: navigator.userAgent,
              languages: navigator.languages,}
           );
  ...
  fbPixelScript.push(
    <noscript key="image" id="facebook-pixel-image">{`
      <img height="1" width="1" style="display:none"
        src="https://www.facebook.com/tr?id=2032207937261066&ev=PageView&noscript=1"
      />
    `}</noscript>
  );
  const allScripts = [fbPixelScript, ...analyticsLibraries, ...mapLibraries];
